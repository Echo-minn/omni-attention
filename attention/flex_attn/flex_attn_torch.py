...
def _convert_mask_to_block_mask(
    ...
    # 例如，假设 mask.shape = [B, H, 7, 9]，Q_BLOCK_SIZE=4，KV_BLOCK_SIZE=8
    # 那么：
    # mask.shape[-2] = 7, 距离4的倍数为最近8，需要pad (8-7)=1，所以 padding_needed_for_multiple(7, 4)=1
    # mask.shape[-1] = 9, 距离8的倍数为最近16，需要pad (16-9)=7，所以 padding_needed_for_multiple(9, 8)=7
    # pad参数为 (0, 7, 0, 1), 分别对应在最后一维pad 7，在倒数第二维pad 1
    ...
    # 这个函数的作用是将输入的block mask张量mask的形状从
    # [B, H, Q//Q_BLOCK_SIZE, Q_BLOCK_SIZE, KV//KV_BLOCK_SIZE, KV_BLOCK_SIZE]
    # 变换为 [B, H, Q//Q_BLOCK_SIZE, KV//KV_BLOCK_SIZE, Q_BLOCK_SIZE, KV_BLOCK_SIZE]，
    # 使得每一个Q/KV的大块都聚合到一起，方便后续对每个block计算是否为全量block或者部分block。
    mask = mask.permute(
        0, 1, 2, 4, 3, 5
    ...
    # 这个地方的作用是根据每个block（Q_BLOCK_SIZE x KV_BLOCK_SIZE）中被mask掉的位置数量，判断当前block属于哪种类型：
    # - 如果separate_full_blocks为True，则返回两个mask：
    #   - partial_blocks: 只要block中有被mask的就为1（但不是全1），否则为0
    #   - full_blocks: block全部为1（即无mask）的为1，否则为0
    # - 如果separate_full_blocks为False，则只关心block里面有没有有效元素（即sum>0），partial_blocks用来指示有未被mask过的位置，返回时full_blocks为None

...
// Note Apply layout swizzle to reduce bank conflicts
...
// Swizzle helper functions to reduce shared memory bank conflicts
...
// Swizzle for Q: row index i, col index j
...
// Swizzle for K: row index i, col index j
template <const int kMmaAtomK = 16>
static __device__ __forceinline__ int swizzle_permuted_K_j(int i, int j) {
  return swizzle_permuted_j<kMmaAtomK, 8>(i, j);
}

// Swizzle for V: row index i, col index j
template <const int kMmaAtomK = 16>
...
omni_attn_swizzle_kernel(
  ...
  // No prefetch support - only shared KV (sequential K/V loading)
  // K and V share the same smem buffer, V overwrites K after K is used
  ...
  // load Q from gmem -> smem, only load once. use CP_ASYNC_CG with swizzling
  {
    ...
    for (int i = 0; i < (kHeadDim / (kNumThreads / Br)); i += 8) {
      int d = load_smem_Q_d + i;
      int d_chunk = d / kMmaAtomK;
      int d_offset = d % kMmaAtomK;
      // Swizzle permutes the offset within the chunk, then add chunk base
      int swizzled_offset = swizzle_permuted_Q_j<kMmaAtomK>(load_smem_Q_Br, d_offset);
      uint32_t load_smem_Q_ptr = (smem_Q_base_ptr + 
                                   (load_smem_Q_Br * (kHeadDim + kPadQ) + 
  ...
  for (int kv_idx = 0; kv_idx < kv_nb; ++kv_idx) {
    ...
    // load K from gmem -> smem, for each KV block(KV_BLOCK_SIZE) with swizzling
    {
    ...
    if (load_smem_K_Bc < Bc) {
      ...
      if (load_gmem_K_Bc < kv_block_end) {
        ...
        for (int i = 0; i < (kHeadDim / (kNumThreads / Bc)); i += 8) {
          int d = load_gmem_K_d + i;
          int d_chunk = d / kMmaAtomK;
          int d_offset = d % kMmaAtomK;
          int swizzled_offset = swizzle_permuted_K_j<kMmaAtomK>(load_smem_K_Bc, d_offset);
    ...
    if (mask_type == BLOCK_MASK_PARTIAL && has_partial_masks && partial_block_masks != nullptr && partial_block_index >= 0) {
      ...
      for (int j = 0; j < kWarpTileSeqLenK; ++j) {
        ...
        // S[2] and S[3] are for rows 8-15 (same row, c2 and c3 are adjacent columns)
        if (row_in_warp >= 8) {
    ...
    // Load V from gmem -> smem (overwrites K's smem after K is used) with swizzling
    {
      ...
      if (load_smem_V_Bc < Bc && load_gmem_V_Bc < kv_block_end) {
        ...
        #pragma unroll
        for (int i = 0; i < (kHeadDim / (kNumThreads / Bc)); i += 8) {
          int d = load_gmem_V_d + i;
          int d_chunk = d / kMmaAtomK;
          int d_offset = d % kMmaAtomK;
          int swizzled_offset = swizzle_permuted_V_j<kMmaAtomK>(load_smem_V_Bc, d_offset);
    ...
    for (int tile_V_Bc = 0; tile_V_Bc < (Bc / kMmaAtomK); ++tile_V_Bc) {
      // load V from smem to registers (k16n8, transposed) with swizzling
      #pragma unroll
      for (int j = 0; j < kWarpTileHeadDimV; ++j) {
        ...
        int d_chunk = lane_smem_V_d / kMmaAtomK;
        int d_offset = lane_smem_V_d % kMmaAtomK;
        int swizzled_offset = swizzle_permuted_V_j<kMmaAtomK>(lane_smem_V_Bc, d_offset);
    ...
    static_assert(kWarpTileSeqLenP == 1);
    {
      ...
      for (int j = 0; j < kWarpTileHeadDimV; ++j) {
        ...
        if constexpr (kOStorageAccFloat32) {
          // (x,y) 0~7->{c0, c1}, (z,w)->8~15 {c2, c3}
          float *D = reinterpret_cast<float *>(&(R_D[0][j][0])); // kWarpTileSeqLenP=1
          ...
          half *D_half = reinterpret_cast<half *>(&(R_D[0][j][0])); // kWarpTileSeqLenP=1
  ...
  static_assert(kWarpTileSeqLenP == 1);
  {
    ...
    //     printf("[omni_attn_swizzle_kernel] batch[0] head[0] row[%d]: row_max=%.8f, row_sum=%.8f\n", 
...
void omni_attn_mma_stages_split_q_shared_kv_swizzle(
  ...
  // Validate partial masks if provided
